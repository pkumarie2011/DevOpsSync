...
public class Main {
	...
	public enum State {
	    Proposed,  // ordinal value: 0
	    Active,    // ordinal value: 1
	    Resolved,  // ordinal value: 2
	    Closed;    // ordinal value: 3
	}
	private static HashMap<Integer, State> hm = new HashMap<Integer, State>();
	public static void main(String... args) throws IOException, GeneralSecurityException {
        ...
        else 
        {
            ...
            for (List row : values) 
            {
            	if(row.get(3)!=null && row.get(3).toString().equals("1"))
            	{
            		...
            		for(Integer issueNumber : workItemNumbers)
            		 {
            			workItemStatusUpdate(issueNumber);
            		 } 
            	}
            }
            System.out.println("\n\nUpdate Completed");
	 ...
	 /**
     * Updates the Status of the workItem depending on the status of its children.
     * 
     * @param  the current Work Item number whose status has to be updated.
	 * @return the final status of the current Work Item. 
     */
	private static State workItemStatusUpdate(int workItemNumber) {
		...
		State workItemState = State.Closed;
		try {
			 ...
			 String currentWorkItemState=jsonObject.getJSONObject("fields").getString("System.State");
			 ...
			 for (int i = 0; i < relationsArray.length(); i++) 
		  		{
		  		    ...
		  		    State childWorkItemState = workItemStatusUpdate(childWorkItemNumber);
		  		    if(workItemState.ordinal()>childWorkItemState.ordinal())
		  		    	workItemState=childWorkItemState;
		  		    }   
		  		}
			 if(flagNodeHasChild==false || currentWorkItemState.equals(workItemState.toString()))
				{
				 if(!hm.containsKey(workItemNumber))
					{
						hm.put(workItemNumber, State.valueOf(currentWorkItemState));
					}
				 return State.valueOf(currentWorkItemState);
				}
				else
				{
					 ...
					 String jsonPatch = "[{\"op\": \"replace\",\"path\": \"/fields/System.State\", \"value\": \""+workItemState+"\"}]";
					 ...
					 System.out.println("\nWork Item "+workItemNumber+" state is now: "+workItemState+".");
				}
		...
		if(!hm.containsKey(workItemNumber))
		{
			hm.put(workItemNumber, workItemState);
		}
     ...
     * Updates the Status of the workItem depending on the status of its children.
     * 
     * @param  the areaPath from which all the workItems have to be extracted.
	 * @return the final workItem IDs list in the areaPath.. 
     */
