...
public class Main {
    ...
    public static void main(String... args) throws IOException, GeneralSecurityException {
        ...
        else 
        {
            ...
            for (List row : values) 
            {
            	if(row.get(1).toString().equals("1"))
            	{
            		...
            		for(Integer issueNumber : workItemNumbers)
            		 {
            			remainingWorkToZero(issueNumber);
            		 } 
            	}
            }
            System.out.println("\n\nUpdate Completed");
    ...
    private static ArrayList<Integer> getItemNumbers(String areaPath)
	{
		...
		String jsonWorkItems = "{\"query\": \"Select [System.Id] From WorkItems Where ([System.State] = 'Closed' OR [System.State] = 'Resolved') AND [System.AreaPath] = '"+areaPath+"'\"}";
	...
	private static void remainingWorkToZero(int workItemNumber) {
		try {
			 ...
			 String workItemState=jsonObject.getJSONObject("fields").getString("System.State");
			 
			 if((workItemState.equals("Closed") || workItemState.equals("Resolved")) && currentRemainingWork!=0)
				{
					 ...
					 String jsonPatch = "[{\"op\": \"replace\",\"path\": \"/fields/Microsoft.VSTS.Scheduling.RemainingWork\", \"value\": "+0+"}]";
					 ...
					 System.out.println("\nWork Item "+workItemNumber+" state is "+workItemState+". Updating the Remaining Work as 0.");
				}
